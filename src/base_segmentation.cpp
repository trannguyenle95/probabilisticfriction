...
BaseSegmentation::BaseSegmentation() : node("~"), tf_listener(tf_buffer) {
  ...
  segment_scene_srv =
      node.advertiseService("material_segmentation/scene_segmentation",
                            &BaseSegmentation::segment_scene_service, this);
  update_params_srv =
      node.advertiseService("material_segmentation/update_base_params",
                            &BaseSegmentation::update_parameters_cb, this);
  pub_pc =
      node.advertise<PointCloud>("material_segmentation/segmented_pc", 1000);
...
bool BaseSegmentation::update_parameters_cb(
    material_segmentation::BaseSegmentationParams::Request &req,
    material_segmentation::BaseSegmentationParams::Response &res) {
...
bool BaseSegmentation::segment_scene_service(
    material_segmentation::SegmentScene::Request &req,
    material_segmentation::SegmentScene::Response &res) {
...
void BaseSegmentation::segment_scene_callback(const PointCloudConstPtr &input) {
  if (pub_pc.getNumSubscribers() != 0) {
    ...
    std::vector<PointCloud> pcs;
    segment_scene(received_cloud_ptr, indices, pcs);
    material_segmentation::SegmentedPointCloud segmented_scene;
